...
% \item  GPA: 3.92 $\cdot$ Blockchain@Columbia member % $\cdot$ ICPC 2024 GNY Regionals participant
% \item GPA: 3.92 $\cdot$ ICPC 2024 GNY Regionals participant
...
\item Wrote proof of concept for and resolved internal "zero-day" vulnerability, preventing injected inputs from being validated
...
\resumeItemListStart
\item Investigated optimizing software systems for satellites using uBPF, a userspace runtime for eBPF
% \item Added support for JIT compilation on uBPF virtual machine for RISC-V architecture
\item Added floating point arithmetic support on uBPF virtual machine in C, introducing new eBPF VM instructions generated by LLVM and modifying the uBPF VM to process them
\resumeItemListEnd
...
\resumeProject
{\href{https://github.com/kristo-og-logi/premking}{PremKing}}
% {Skills: React Native $\cdot$ Go $\cdot$ AWS EC2 $\cdot$ OAuth $\cdot$ Postgres $\cdot$ App Store \& Testflight $\cdot$ Lets Encrypt $\cdot$ Nginx}
{\href{https://github.com/kristo-og-logi/premking}{github.com/logi-og-kristo/premking \footnotesize{\faGithub}}}
\resumeItemListStart
\item Launched open source full-stack mobile app on \href{https://apps.apple.com/is/app/premking/id6476774713}{the App Store}, managing $\approx$ 100 monthly users, using React Native, Golang backend REST API deployed on AWS, connected to PostgresSQL database, with push notifications
